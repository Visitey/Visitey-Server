...
def compat_serializer_method_field(method_name=None):
    """ method_name changed in DRF > 3. See http://www.django-rest-framework.org/topics/3.0-announcement/#optional
    -argument-to-serializermethodfield. """
    if DRFVLIST[0] >= 3:
...
def compat_thread_serializer_set():
    """ We create the Thread manually and must assign it to the serializer. DRF 3 uses serializer.instance while DRF
    2 uses serializer.object """
    if DRFVLIST[0] >= 3:
        return "instance"
    else:
        return "object"
...
def compat_get_paginated_response(view, page):
    ...
    if DRFVLIST[0] == 3 and DRFVLIST[1] >= 1:
        from rest_chat.serializers import ComplexMessageSerializer  # circular import
...
def compat_pagination_messages(cls):
    """
    For DRF 3.1 and higher, pagination is defined at the paginator level (see
    http://www.django-rest-framework.org/topics/3.2-announcement/). For DRF 3.0 and lower, it can be handled at the
    view level.
    """
    if DRFVLIST[0] == 3 and DRFVLIST[1] >= 1:
