...
class ThreadManager(models.Manager):
    ...
    def get_threads_for_participant(self, participant_id):
        """ Gets all the threads in which the current participant is or was involved. The method does not exclude
        threads where the participant has left. """
        return Thread.objects. \
            filter(participants__id=participant_id). \
            distinct()

    def get_threads_where_participant_is_active(self, participant_id):
        """ Gets all the threads in which the current participant is involved. The method excludes threads where the
...
class Participation(models.Model):
    ...
    date_last_check = models.DateTimeField(null=True,
                                           blank=True)  # a timestamp to be set when a participant reads a thread


class MessageManager(models.Manager):

    def return_daily_messages_count(self, sender):
        """ Returns the number of messages sent in the last 24 hours so we can ensure the user does not exceed his
        messaging limits """
    ...
    def get_lasts_messages_of_threads(self, participant_id, check_who_read=True, check_is_notification=True):
        ...
        # we get the last message for each thread we must query the messages using two queries because only Postgres
        # supports .order_by('thread', '-sent_at').distinct('thread')
        # TODO CHANGE FOR POPSTGRECOMPLIENT QUERY
